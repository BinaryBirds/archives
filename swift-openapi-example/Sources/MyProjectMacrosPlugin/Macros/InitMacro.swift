...
extension InitMacro {
    public enum AccessLevel: String, Equatable, Comparable {
        ...
        public static func < (
            lhs: InitMacro.AccessLevel,
            rhs: InitMacro.AccessLevel
        ) -> Bool {
...
extension InitMacro {
    enum Error: Swift.Error, CustomStringConvertible {
        ...
        var description: String {
            ...
            case .invalidAccessLevel:
                return
                    "Invalid access level. Macro only works with open, public or internal."

            case .invalidAccessLevelHierarchy:
                return
                    "The requested access level is higher than the object's access level."
            ...
            case let .cannotInferType(variable):
                return
                    "Could not infer the type for \(variable). Please specify it explicitly."

            case let .excludingNonInitialisedProperty(name):
                return
                    "Property \(name) was excluded without being initialised."
...
extension InitMacro {
        ...
        let parametersAndStatements = try declaration.memberBlock.members
            .reduce(into: [(String, String)]()) { partialResult, member in
                guard
                    ...
                    let bindings = syntax.bindings.as(
                ...
                if keysToExclude.contains(identifier) {
                    guard pattern.initializer != nil else {
                        throw Error.excludingNonInitialisedProperty(
                            named: identifier
                        )
                    }
                ...
                if  // Initialised constants should not be included.
                syntax.bindingSpecifier.tokenKind == .keyword(.let)
                    && pattern.initializer != nil
                    // Computed property
                ...
                let type =
                    (pattern.typeAnnotation?.as(TypeAnnotationSyntax.self))?
                    .type
                if var typeValue = type?.description.trimmingCharacters(
                    in: .whitespacesAndNewlines
                ) ?? extractType(from: pattern) {
                    typeValue =
                        type?.is(FunctionTypeSyntax.self) == true
                        ? "@escaping " + typeValue : typeValue

                    let assignment = "self.\(identifier) = \(identifier)"
                    let parameter =
                        if let defaultValue = defaultValues[identifier] {
    ...
    private static func extractType(from binding: PatternBindingSyntax)
        -> String?
    {
        ...
        if let sequenceSyntax = binding.initializer?.value.as(
            SequenceExprSyntax.self
        ),
            let castedType = sequenceSyntax.elements.first(where: {
                $0.kind == .typeExpr
            })?.description
        {
            let isOptional =
                sequenceSyntax.elements
        ...
        if let arraySyntax = initializer.value.as(ArrayExprSyntax.self) {
            ...
            type =
                arraySyntax.elements.contains(where: {
                    $0.expression.is(NilLiteralExprSyntax.self)
                }) ? "\(type)?" : type
            return "[\(type)]"
        }

        if let dictionarySyntax = initializer.value.as(
            ...
            guard
                let dictionaryListSyntax = dictionarySyntax.content.as(
                    DictionaryElementListSyntax.self
                ),
                let anyElement = dictionaryListSyntax.first(where: {
                    !$0.value.is(NilLiteralExprSyntax.self)
                }),
            ...
            valueType =
                dictionaryListSyntax.contains(where: {
                    $0.value.is(NilLiteralExprSyntax.self)
                }) ? "\(valueType)?" : valueType
    ...
    private static func getAccessLevel(of node: AttributeSyntax) -> AccessLevel
    {
        let accessLevel: AccessLevel =
            switch node
                ...
                .first(where: {
                    $0.label?.tokenKind == .identifier("accessLevel")
                })?
    ...
    private static func getAccessLevel(of declaration: some DeclGroupSyntax)
        -> AccessLevel
    {
    ...
    private static func getDefaultValues(from node: AttributeSyntax) -> [String:
        String]
    {
        return node
            .arguments?.as(LabeledExprListSyntax.self)?
            .first(where: {
                $0.label?.tokenKind == .identifier("defaultValues")
            })?
